...
export class PostFX {
  ...
  render() {
    ...
    if (this.waterMaskObjects && this.waterMaskObjects.length > 0) {
      const savedMaskVis = new Map()
      scene.traverse((child) => {
        if (!child || (!child.isMesh && !child.isBatchedMesh && !child.isInstancedMesh &&
            !child.isLine && !child.isLineSegments && !child.isPoints)) return
        const isMaskObj = this.waterMaskObjects.some(o => o && (o === child || (typeof o.getObjectById === 'function' && o.getObjectById(child.id))))
        if (!isMaskObj) {
          if (typeof child.visible === 'boolean') savedMaskVis.set(child, child.visible)
          child.visible = false
        }
      })
      ...
      for (const [child, vis] of savedMaskVis) {
        if (child) child.visible = vis
      }
    }
    ...
    scene.traverse((child) => {
      if (!child || (!child.isMesh && !child.isLine && !child.isLineSegments && !child.isPoints)) return
      const isOverlay = overlayObjects.some(o => o && (o === child || (typeof o.getObjectById === 'function' && o.getObjectById(child.id))))
      if (!isOverlay) {
        if (typeof child.visible === 'boolean') savedVisibility.set(child, child.visible)
        child.visible = false
      }
    })
    ...
    if (waterObjects && waterObjects.length > 0) {
      const savedWaterVis = new Map()
      scene.traverse((child) => {
        if (!child || (!child.isMesh && !child.isLine && !child.isLineSegments && !child.isPoints)) return
        const isWater = waterObjects.some(o => o && (o === child || (typeof o.getObjectById === 'function' && o.getObjectById(child.id))))
        if (!isWater) {
          if (typeof child.visible === 'boolean') savedWaterVis.set(child, child.visible)
          child.visible = false
        }
      })
      ...
      for (const [child, vis] of savedWaterVis) {
        if (child) child.visible = vis
      }
    }
